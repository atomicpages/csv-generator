...
const genData = require('./vendor/gendata');

eval(fs.readFileSync(path.resolve('./vendor/csvsup.js')) + '');
eval(fs.readFileSync(path.resolve('./vendor/strsup.js')) + '');
...
module.exports = function (name, columns, options, headers) {
    ...
    stream.on('finish', () => {
        console.log(`Successfully wrote ${chalk.green(options.rows)} to ${path.resolve(name)} in ${(new Date().getTime() - start) / 1000}s`);
    });
