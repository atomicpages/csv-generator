...
/**
 * Generates a timestamp function based on arguments.
 * @param {string} args Comma-separated arguments: beginDate, endDate, incrementSeconds.
 * @return {function} Generator function for timestamps.
 */
function createTimestampGenerator(args) {
    const parts = args.split(',');
    const beginDate = parts[0] ? new Date(parts[0].trim()).getTime() || 0 : 0;
    const endDate = parts[1] ? new Date(parts[1].trim()).getTime() || Date.now() : Date.now();
    const increment = parts[2] ? parseInt(parts[2].trim(), 10) * 1000 : 0;
    let currentTimestamp = beginDate;

    return function() {
        if (increment === 0) {
            const randomTime = Math.floor(Math.random() * (endDate - beginDate + 1)) + beginDate;
...
module.exports = function (name, columns, options, headers) {
    ...
    // Preprocess columns to support custom timestamp generator
    const processedColumns = columns.map(col => {
        if (typeof col === 'string' && col.indexOf('timestamp(') === 0) {
            const match = col.match(/^timestamp\((.*)\)$/);
            return match ? createTimestampGenerator(match[1]) : col;
        }
        return col;
    });
    ...
    return new Promise(resolve => {
        for (let i = 0; i < options.rows; i += options.chunks) {
            ...
            let csvStringPart = genData(processedColumns, size);
